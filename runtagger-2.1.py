...
def tag_sentence(test_file, model_file, out_file):
    ...
    ##### load model #####
        ...
        # }, model_file)
        # LSTMTagger(EMBEDDING_DIM, HIDDEN_DIM, len(word_to_ix), len(tag_to_ix), PAD_IDX, DROPOUT_RATE).to(device)
    ...
    with torch.no_grad():
        ...
        for i in range(0, len(test)//test_batch_size+1):
            ...
            model.hidden = model.init_hidden(test_batch_size)
